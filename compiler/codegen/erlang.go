...
func (generator *ErlangGenerator) Generate(program *ast.Program) string {
	var builder strings.Builder

	// Generate module header
	builder.WriteString(generator.generateModuleHeader())

	// Find main function and generate it
	hasMain := false
	for _, stmt := range program.Statements {
		if funcStmt, ok := stmt.(*ast.FunctionStatement); ok {
			if funcStmt.Name.Value == "main" {
				hasMain = true
				functionCode := generator.generateFunction(funcStmt)
	...
	// If no main function found, generate error
	if !hasMain {
		...
		// Generate default start function
...
func (generator *ErlangGenerator) generateFunction(function *ast.FunctionStatement) string {
	var builder strings.Builder
...
func (generator *ErlangGenerator) generateStatementSequence(statements []ast.Statement) string {
	if len(statements) == 0 {
...
func (generator *ErlangGenerator) generateStatement(statement ast.Statement) string {
	if statement == nil {
		return ""
	}
	...
	case *ast.ReturnStatement:
		return generator.generateReturnStatement(node)
	case *ast.FunctionStatement:
		// Skip function statements in body (they are handled at top level)
		return ""
	default:
		generator.errors.Add(
			errors.SyntaxError,
...
func (generator *ErlangGenerator) generateLetStatement(letStmt *ast.LetStatement) string {
	variable := generator.naviaryToErlangVariable(letStmt.Name.Value)
...
// generateReturnStatement converts return statement
func (generator *ErlangGenerator) generateReturnStatement(returnStmt *ast.ReturnStatement) string {
	if returnStmt.ReturnValue != nil {
		value := generator.generateExpression(returnStmt.ReturnValue)
		return fmt.Sprintf("%s%s", generator.indentString(), value)
	}
	return fmt.Sprintf("%sok", generator.indentString())
}

// generateBlockStatement converts a block of statements to Erlang
func (generator *ErlangGenerator) generateBlockStatement(block *ast.BlockStatement) string {
	var builder strings.Builder
...
func (generator *ErlangGenerator) generateExpressionStatement(exprStmt *ast.ExpressionStatement) string {
...
func (generator *ErlangGenerator) generateExpression(expression ast.Expression) string {
	if expression == nil {
		return "undefined"
	}
...
// generateIntegerLiteral converts an integer literal to Erlang
func (generator *ErlangGenerator) generateIntegerLiteral(integer *ast.IntegerLiteral) string {
	// Remove underscores from the value if present (e.g., 1_000 -> 1000)
	cleanValue := strings.ReplaceAll(integer.Value, "_", "")
	return cleanValue
}

// generateIdentifier converts an identifier to Erlang (uppercase for variables)
func (generator *ErlangGenerator) generateIdentifier(identifier *ast.Identifier) string {
...
func (generator *ErlangGenerator) generateBinaryExpression(binary *ast.BinaryExpression) string {
...
func (generator *ErlangGenerator) generateCallExpression(call *ast.CallExpression) string {
	// Get function name
	funcName := ""
	...
	return fmt.Sprintf("%s(%s)", funcName, strings.Join(arguments, ", "))
}

// generatePrintCall handles the special 'print' built-in
func (generator *ErlangGenerator) generatePrintCall(call *ast.CallExpression) string {
...
func (generator *ErlangGenerator) GenerateToFile(program *ast.Program) (string, error) {
	if program == nil {
		generator.errors.Add(
			errors.SyntaxError,
			1,
	...
	if generator.errors.HasErrors() {
		return "", fmt.Errorf("code generation failed with errors")
	}
